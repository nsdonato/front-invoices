...
export const InvoiceItem = <C extends ElementType>({
  ...
  return (
    <MutableElement
      {...props}
      className="relative bg-brand bg-white lg:max-w-3xl md:max-w-2xl max-w-sm py-4 lg:pr-6 md:pr-6 rounded-lg mx-auto my-4 dark:bg-brand-gray-dark grid md:grid-rows-1 lg:grid-rows-1 lg:grid-cols-5 md:grid-cols-5 grid-cols-2 items-center text-center shadow-md cursor-pointer text-sm border transition-all hover:border-brand-violet"
    >
      ...
      <h4 className="font-bold text-base lg:order-4 md:order-4 order-4 dark:text-brand-white">
        {normalizedAmount}
      </h4>
      <div
        className={`${invoiceBackgrounds[status]} ${
          status === "draft" ? "dark:bg-brand-violet-highlight" : ""
        } dark:bg-opacity-5 max-w-[104px] mx-auto w-full relative py-3 rounded-md lg:order-5 md:order-5 order-3 row-span-2`}
      >
        <p
          className={`${invoiceTexts[status]} ${
            status === "draft" ? "dark:text-brand-white" : ""
          } relative font-bold`}
        >
          <span
            className={`absolute top-1 before:absolute ${invoiceDots[status]} ${
              status === "draft" ? "dark:before:bg-brand-white" : ""
            } before:rounded-full before:p-[3px] before:right-1`}
          ></span>
          {normalizedStatus}
        </p>
      </div>
      <span className="ml-2 w-fit right-4 absolute lg:block lg:order-6 md:block md:order-6 hidden">
        <Arrow />
      </span>
    </MutableElement>
  );
